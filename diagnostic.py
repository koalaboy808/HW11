...
def f06(string1, string2):
    char_string1 = len(string1)
    char_string2 = len(string2)
    difference = char_string2 - char_string1

    if (char_string2 - char_string1) >= 0:
        winner = string2
        loser = string1
        percentage = str(round(float(char_string1)/char_string2*100,2)) + "%"
    else:
        winner = string1
        loser = string2
        percentage = str(round(float(char_string2)/char_string1*100,2)) + "%"

    print("{0} is longer than {1} by {2} chars").format(winner,loser,abs(difference))
    print("{0} has only {1} the number of chars of {2}").format(loser,percentage,winner)
